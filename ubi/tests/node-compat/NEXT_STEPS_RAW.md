...
Goal: run Node’s test files **unchanged** (no copies, no edits) with ubi, so behavior and paths match Node.
...
They use: redirect of `node/test/common/` to ubi's minimal `common/` (index, fixtures, tmpdir) and `NODE_TEST_DIR` for fixtures. Builtins resolve through the same runtime builtins as normal execution. The runner is the CI gate for raw Node tests.

---

## 1. Run tests from the Node test directory

**Current:** Tests live under `ubi/tests/node-compat/` and are run via a path like `.../node-compat/parallel/test-require-cache.js`. That forces our own `common/` and `builtins/` and the "node-compat" → "test" path hack in JSON errors.

**Target:** Run the **same** files as Node, e.g.:

- `ubi /path/to/node/test/parallel/test-require-cache.js`
...
- Add a test runner (or CTest) that invokes the ubi binary with a path into the vendored **node/test** tree (e.g. `node/test/parallel/test-require-cache.js`).
- Ensure the loader uses the **script’s directory** as the base for resolution so `../common` and `../fixtures` resolve under `node/test/` (no copy of tests into ubi).

---

## 2. Resolve Node’s common and fixtures from node/test

**Current:** We ship our own `ubi/tests/node-compat/common/` and `fixtures/`.
...
- If we keep a “compat” mode that runs from `ubi/tests/node-compat/`, keep that as an alternative; for “raw” mode, only run from `node/test/` and rely on Node’s layout.

---

## 3. Provide Node-compatible builtins (assert, path, fs, module)

**Current:** `require('assert')` and `require('path')` resolve through `ubi/src/builtins` (same path used in non-test runtime).
...
- If we keep a compat tree under ubi, port or reimplement `common/tmpdir.js` and `common/fixtures.js` to match Node’s API and rely on our builtins for `fs` and `path`.
...
- Remove the `node-compat` → `test` replacement in `ubi_module_loader.cc` once the runner uses the Node test directory.
...
- Add a script or CTest target that:
  ...
  - Runs ubi with that script and passes/fails on exit code.
...
1. **Runner from node/test** – Run ubi with `node/test/parallel/test-require-cache.js` (and test-require-json.js) and fix resolution so `../common` and `../fixtures` resolve under `node/test/`. Remove the JSON error path hack once paths are correct. **Done.**
...
This gets you to “tests run exactly as in Node” (same files, same require graph, same behavior) without maintaining a separate copy under `ubi/tests/node-compat/` for those tests.
