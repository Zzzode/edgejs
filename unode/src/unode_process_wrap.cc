...
void EmitOnExit(ProcessWrap* wrap, int32_t exit_code, int32_t signal_code, bool has_signal) {
  ...
  UnodeMakeCallback(wrap->env, self, onexit, 2, argv, &ignored);
}
...
napi_value ProcessSpawn(napi_env env, napi_callback_info info) {
  ...
  bool use_real_async = IsTruthyProperty(env, argv[0], "__unodeRealAsync");
  if (!use_real_async) {
      ...
      for (uint32_t i = 0; i < len; i++) {
        ...
        if (t == napi_object) {
          ...
          if (type == "pipe" || type == "overlapped" || type == "wrap") {
            ...
            uv_stream_t* stream = UnodePipeWrapGetStream(env, handle_value);
            if (stream != nullptr) {
              if (type == "wrap") {
