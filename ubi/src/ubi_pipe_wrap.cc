...
const UbiStreamBaseOps kPipeOps = {
    PipeGetHandle,
...
void OnConnectDone(uv_connect_t* req, int status) {
  ...
  napi_value pipe_obj = cr->pipe != nullptr ? UbiStreamBaseGetWrapper(&cr->pipe->base) : nullptr;
  napi_value argv[5] = {
...
napi_value PipeCtor(napi_env env, napi_callback_info info) {
  ...
  int32_t provider = kUbiProviderPipeWrap;
  if (socket_type == kPipeServer) provider = kUbiProviderPipeServerWrap;
  UbiStreamBaseInit(&wrap->base, env, &kPipeOps, provider);
...
napi_value PipeOpen(napi_env env, napi_callback_info info) {
  ...
  return UbiStreamBaseMakeInt32(env, uv_pipe_open(&wrap->handle, static_cast<uv_file>(fd)));
}
...
void OnConnection(uv_stream_t* server, int status) {
  ...
  if (status == 0) {
    ...
    napi_value type_arg = UbiStreamBaseMakeInt32(env, kPipeSocket);
    ...
    argv[0] = UbiStreamBaseMakeInt32(env, rc);
    argv[1] = client_obj;
  }
...
napi_value PipeWriteBuffer(napi_env env, napi_callback_info info) {
  ...
  return UbiLibuvStreamWriteBuffer(&wrap->base, argv[0], argv[1], send_handle, send_handle_obj);
}
...
napi_value PipeFchmod(napi_env env, napi_callback_info info) {
  ...
  return UbiStreamBaseMakeInt32(env, uv_pipe_chmod(&wrap->handle, mode));
}
...
napi_value AcceptPendingHandleForIpc(UbiStreamBase* base) {
  ...
  if (env == nullptr || wrap == nullptr) return UbiStreamBaseUndefined(env);

  if (uv_pipe_pending_count(&wrap->handle) <= 0) return UbiStreamBaseUndefined(env);
  ...
  if (pending_type == UV_TCP) {
    ...
    napi_value arg = UbiStreamBaseMakeInt32(env, 0);
    if (tcp_ctor == nullptr ||
        arg == nullptr ||
    ...
    PipeBindingState* state = GetBindingState(env);
    napi_value ctor = state != nullptr ? GetRefValue(env, state->pipe_ctor_ref) : nullptr;
    napi_value arg = UbiStreamBaseMakeInt32(env, kPipeSocket);
    if (ctor == nullptr ||
        arg == nullptr ||
    ...
    accept_target = reinterpret_cast<uv_stream_t*>(UbiUdpWrapGetHandle(env, handle_obj));
  } else {
    return UbiStreamBaseUndefined(env);
  }

  if (accept_target == nullptr) return UbiStreamBaseUndefined(env);
