...
- The only intended source adaptation is replacing direct V8 API usage with
  N-API usage.
- Hard enforcement: `ubi/src` must not include V8 headers (`v8.h`,
  `libplatform/libplatform.h`) and must not use `v8::` symbols.
- Any required V8 host/bootstrap code must live outside `ubi/src`.

## Phase 0 - Project Bootstrap

### Deliverables
- `ubi` directory structure and runtime docs.
...
### Exit Criteria
- `ubi` can run `hello.js` and returns exit code `0`.
...
### Deliverables
- Script execution pipeline (`ubi script.js`).
