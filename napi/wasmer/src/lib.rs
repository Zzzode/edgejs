...
fn call_guest_callback(store: &mut impl wasmer::AsStoreMut, table: &Table, wasm_fn_ptr: u32, data_val: u64) -> u32 {
    ...
    match func.call(
        store,
        &[Value::I32(UNOFFICIAL_ENV_HANDLE), Value::I32(data_val as i32)],
    ) {
...
fn with_top_level_callback_state<R, F>(store: &mut Store, table: Option<Table>, f: F) -> R
where
            ...
            if let Some(repo_root) = repo_root {
                ...
                let node_lib_fs: Arc<dyn FileSystem + Send + Sync> = Arc::new(
                    virtual_fs::host_fs::FileSystem::new(host_handle.clone(), node_lib_dir.clone())
                        .with_context(|| format!("failed to create host fs for {}", node_lib_dir.display()))?,
                );
                TmpFileSystem::mount(&root_fs, "/node-lib".into(), &node_lib_fs, "/".into())
                    .with_context(|| format!("failed to mount {} at /node-lib", node_lib_dir.display()))?;
                ...
                if node_deps_dir.is_dir() {
                    ...
                    let node_deps_fs: Arc<dyn FileSystem + Send + Sync> = Arc::new(
                        virtual_fs::host_fs::FileSystem::new(host_handle.clone(), node_deps_dir.clone())
                            .with_context(|| format!("failed to create host fs for {}", node_deps_dir.display()))?,
                    );
                    TmpFileSystem::mount(&root_fs, "/node/deps".into(), &node_deps_fs, "/".into())
                        .with_context(|| format!("failed to mount {} at /node/deps", node_deps_dir.display()))?;
                }
            }
