...
let asyncCbCount = 0;
let tsfnCbCount = 0;
let tsfnFinalizeCount = 0;
__tid.asyncWorkCallback(() => { asyncCbCount++; });
__tid.testThreadsafeFunction(() => { tsfnCbCount++; }, () => { tsfnFinalizeCount++; });
if (asyncCbCount !== 1) throw new Error('asyncWorkCallback');
)JS"));
}
