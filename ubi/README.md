...
`ubi` is a runtime project that aims to replace Node.js while keeping N-API as
the core boundary. Unlike Node internals that integrate directly with V8 in many
paths, `ubi` system bindings should be implemented through `napi/v8` APIs.
...
- `ubi` source and tests should be ported from Node as fully as possible.
...
- Hard boundary: files under `ubi/src` must never include V8 headers
  (`v8.h`, `libplatform/libplatform.h`) or use `v8::` symbols.
- Host/bootstrap code that requires V8 must live outside `ubi/src` (for
  example, under `napi/v8`), while `ubi/src` remains N-API/Node-API only.
...
Detailed milestones and test gates are in `ubi/ROADMAP.md`.

1. **Bootstrap**
   - `ubi` executable that creates an environment through `napi/v8`.
